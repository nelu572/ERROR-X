Shader "ERROR-X/MenuFX/TMP Text Glitch" {
...
SubShader {
	...
	Pass {
		...
		fixed4 PixShader(pixel_t input) : SV_Target
		{
			...
			float row = floor(input.atlas.y * _StripFrequency);
			float active = step(1.0 - _TriggerRate, Hash11(row + floor(t * 6.0)));
			float offset = (Hash11(row * 1.73 + floor(t * 11.0)) - 0.5) * _Intensity * active;
			float2 glitchUV = input.atlas + float2(offset, 0.0);
			float c = tex2D(_MainTex, glitchUV).a;
			...
			float3 n = GetSurfaceNormal(glitchUV, weight, dxy);
			...
			float edgeClip = step(0.0, glitchUV.x) * step(glitchUV.x, 1.0);
			float artifact = lerp(1.0, edgeClip, saturate(active * _ArtifactStrength));
			faceColor.a *= artifact;
...
CustomEditor "ErrorX.Editor.MenuTmpTextGlitchShaderGUI"
}
